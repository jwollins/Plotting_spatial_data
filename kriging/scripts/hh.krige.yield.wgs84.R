...
## 02 Data ####
source(file = "scripts/shp.data.R")



### Read CSV datasets ####
 
csvdat <- read.csv(file = "kriging/data/yield_data/bean.data.wgs84.csv", 
                   header = TRUE)
...
datCON <- csvdat %>% filter(Treatment == "Conventional")

# filter to CA system
datCA <- csvdat %>% filter(Treatment == "Conservation")


gghistogram(csvdat$t_ha)


# 01 PLOT POINT DATA ####

#open png for file save and define size and resolution
png(paste("kriging/plots/", "point.plot", ".png", sep=""),
...
ggplot()+ geom_point(data = df.image, aes(y=Latitude, x=Longitude, color=Values),
  ...
  scale_colour_gradient(
    ...
    guide = "colourbar",
    aesthetics = "colour") +
  geom_point(data = csvdat,  aes(x = x, y = y), 
             color="black", 
             alpha = 1) +
...
#open png for file save and define size and resolution
png(paste("kriging/plots/", "interpol.grid.plot", ".png", sep=""),
    width=1000, height=1000, res=150)
...
coordinates(f_grid) <- ~ x + y # step 3 above
coordinates(csvdat) <- ~ x + y # step 3 above
...
kriging_result = autoKrige(t_ha ~ 1, csvdat, f_grid)

#open png for file save and define size and resolution
png(paste("yield/hand.harvest/plots/", "t_ha_krige_plot", ".png", sep=""),
    width=1000, height=1000, res=150)
...
con.krig = autoKrige(t_ha ~ 1, datCON, f_grid)
...
ca.krig = autoKrige(t_ha ~ 1, datCA, f_grid)
